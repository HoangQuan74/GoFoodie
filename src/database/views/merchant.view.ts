...
@ViewEntity({
  expression: `
    SELECT 
      ...
      sum(case when s.status = '${EStoreStatus.Approved}' then 1 else 0 end) AS approved_store_number,
      sum(case when s.status = '${EStoreStatus.Rejected}' then 1 else 0 end) AS unapproved_store_number
    FROM merchants m
